...
export class TensorService {
  ...
  private static imageToTensor(imageBuffer: ArrayBuffer): Tensor3D {
    const image: TiffIfd = tiff.decode(imageBuffer)[0];
    const tensor = tf.tensor3d(Array.from(image.data), [image.height, image.width, image.get('SamplesPerPixel')], 'float32');
  ...
  initializeTensors(imageBuffers: ArrayBuffer[]): void {
    const tensors = imageBuffers.map(imageBuffer => TensorService.imageToTensor(imageBuffer));
  ...
  convertToImageData(tensors: Tensor3D[], channelNr: number): Promise<ImageData>[] {
    return tensors.map(tensor => {
      const channel = tensor.slice([0, 0, channelNr], [tensor.shape[0], tensor.shape[1], 1]);
